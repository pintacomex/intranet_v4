class ComisionesController < ApplicationController
  ...
  def index
    ...
    if params.has_key?("id_vendedor")
      ...
      @current_vendedor_name = vendedor_name['Nombre'] rescue ''
      return redirect_to root_path, notice: "Vendedor no encontrado" if @current_vendedor_name == ''
    end
    @sql = ""
    @num_empleado = params
    if @categoria == 5
      ...
      sql = "select DISTINCT IdVendedor, Nombre from #{@nomDbPdv}catcomisionistas where IdVendedor > 0 and fechaterm = '' and categoria = 0 order by IdVendedor"
      vendedores = @dbEsta.connection.select_all(sql)
      @cat_vendedores = []
      vendedores.map { |vendedor| @cat_vendedores << ["#{vendedor['IdVendedor']}-#{vendedor['Nombre']}", vendedor['IdVendedor']] }
      sql = "select DISTINCT IdVendedor, Nombre from #{@nomDbPdv}catcomisionistas where IdVendedor > 0 and fechaterm = '' and categoria = 1 order by IdVendedor"
      vendedores = @dbEsta.connection.select_all(sql)
      @cat_sucursales = []
      vendedores.map { |vendedor| @cat_sucursales << ["#{vendedor['IdVendedor']}-#{vendedor['Nombre']}", vendedor['IdVendedor']] }
      sql = "select DISTINCT IdVendedor, Nombre from #{@nomDbPdv}catcomisionistas where IdVendedor > 0 and fechaterm = '' and (categoria = 2 or categoria = 3) order by IdVendedor"
      vendedores = @dbEsta.connection.select_all(sql)
      @cat_subzona = []
      vendedores.map { |vendedor| @cat_subzona << ["#{vendedor['IdVendedor']}-#{vendedor['Nombre']}", vendedor['IdVendedor']] }
    else
      sql = "(select Mes, Sucursal, (FechaUltimaVtaSuc) as ultFechaProcesada, diasVenta as DiasOperados, (Idvendedor) as IdEmpleado, 'VendedorV2' as Tipo from ComisXVtasAsesor where Idvendedor = #{User.sanitize(num_empleado)}) union (select Mes, Sucursal, (FechaUltimaVtaSuc) as ultFechaProcesada, diasVenta as DiasOperados, (Idvendedor) as IdEmpleado, 'EncargadoV2' as Tipo from ComisXVtasEncargado where Idvendedor = #{User.sanitize(num_empleado)}) union (select Mes, Zona as Sucursal, (FechaUltimaVta) as ultFechaProcesada, diasVenta as DiasOperados, (Idvendedor) as IdEmpleado, 'RegionalV2' as Tipo from ComisXVtasRegional where Idvendedor = #{User.sanitize(num_empleado)}) order by mes desc"
      @meses_vendedor = @dbEsta.connection.select_all(sql)
      @sql = sql
    end
  end
  ...
  def show_report
    ...
    if @categoria == '5' || tipo == "Vendedor"
      ...
      sql = "select *, (FechaUltimaVtaSuc) as ultFechaProcesada, diasVenta as DiasOperados, diasFaltantes as DiasPorTranscurrir from ComisXVtasEncargado where Idvendedor = #{User.sanitize(num_empleado)} and mes = #{User.sanitize(params[:mes])} and sucursal = #{User.sanitize(params[:sucursal])}"
      @comision_mes = @dbEsta.connection.select_all(sql).first
      sql = "select * from objvtassuc where sucursal = #{User.sanitize(params[:sucursal])} and mes = #{User.sanitize(params[:mes])}"
      @objetivos_mes = @dbPdv.connection.select_all(sql).first
      sql = "select * from sucursal where Num_suc = #{User.sanitize(@comision_mes['Sucursal'])}"
  ...
  def print_pdf
    ...
    respond_to do |format|
      ...
      format.pdf do
        ...
        pdf = ComisionesPdf.new(vendedor, @current_vendedor_name, mes, tipo_vista, nombre_suc, objetivos, dias_transcurridos, categoria)
